...
::: {.column width="30%"}
Connections often use nonlinear _activation functions_ for $\sigma(\cdot)$.
:::
...
There are a variety of nonlinear functions. Historically, a logistic/sigmoid was popular but rReLU and its variants are mostly used. 
...
act_functions <-
  bind_rows(
    ...
    tibble(
      input = lp_grid,
      output = as.numeric(torch::nn_softplus()(lp_grid)),
      activation = "softplus"
    )
  )
...
- High maintenance
  ...
  - Need GPU for complx models
- Perfectly capable of overfitting
- Accurate but probably not well-calibrated.
- Can be highly performant but often not optial
...
# prediction: 
grid_mlp <- 
  augment(init_mlp, new_data = cls_grid) |> 
...
There are a variety of tools that have trickeled from image-based nerual networks to tabular data models: 
...
These help solve issues of vanishing or exploding gradients.
:::
...
As models and/or data sizes increase, it becomes nessecary to use a GPU for computations. 

There are two main GPU computational fraemworks: 

- [CUDA](https://en.wikipedia.org/wiki/CUDA): from NVidia GPUs
- [MPS](https://en.wikipedia.org/wiki/Metal_(API)): Apple CPUs

_Most_ focus is on CUDA instructions but MPS works very well (outside of a few bugs). 
...
You _can_ run explicit (i.e., multiprocess) parallel processing with GPU but it is very important to ensure that there is enought GPU memory to send simultaneous jobs at once. 
...
R and torch seeds were set each iteration. 
:::
...
For many nonclinical assays, there are _in silico_ models that are usually built for med chem folks to predict with. 

<br>

With assays that are applicable everywhere (e.g., ADME, toxicology, etc), _global models_ across all programs and therapeudic areas are used. 

<br>

However, for some new projects with HTS hits, _local_ computational chemistry models are creates an very small data sets of _highly relevant_ compounds. 

<br> 

These small models are beielved to be better since they have the *correct context* for their program. 

## Attention

This is a method to improve specific predictions that are made with very large neural networks that are created with extremely disparate training set points. 

<br>

Attention helps focus the internal embeddings to bias the network towards its relavant parts. For example, attention can adress the question: 
...
For tabular data, we can compute column- or row-attention. 

During training: 

- Column attention takes the data in the current batch, and computes new embeddings _per feature_ and _across data points_. 
- Row attention adjusts the mebeddings _for each row_ and _across embedding columns_. 

At prediction time, no new parameters are estimated. The predictions are influence by the other rows being predicted. 

## SAINT

SAINT is a deep learning model that alternates between column and row attention. 

- The size of an attention _head_ is how many simulataneous attention computations are being computed ("width")
- An attention _block_ is sets of seqeuntial heads are executed ("depth"). 
...
We still need a "training set" to trigger _in-context learning_. 
...
There are different tabular foundations models. the two foremost models are TabPFN and TabICL, whose results are very similar. 

More are popping up all of the time. 
